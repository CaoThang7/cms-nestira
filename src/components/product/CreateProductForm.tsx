...
import { createProductSchema, CreateProductFormType } from "@/schemas/product";
...
const CreateProductForm = () => {
  ...
  const totalPriceError = getErrorMessage(errors.totalPrice?.message, activeLang);
  const productCodeError = getErrorMessage(errors.productCode?.message, activeLang);
  const categoryNameError = getErrorMessage(errors.categoryId?.message, activeLang);
  ...
  const onSubmit: SubmitHandler<CreateProductFormType> = async (data) => {
    ...
    try {
      if (!user || user.role !== "admin") {
        toast("Only admins are allowed to create a product.", {
          icon: "⚠️",
      ...
      const dataRes = await createProduct(data);
      ...
      setValue("origin.en", "");
      setValue("origin.vi", "");
      setValue("material.en", "");
      setValue("material.vi", "");
      setValue("size", "");
      setValue("description.en", "");
      setValue("description.vi", "");
      setValue("specifications.en", "");
      setValue("specifications.vi", "");
  ...
  }, [activeLang, getValues, setValue, data]);
